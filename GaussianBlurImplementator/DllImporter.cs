...
namespace GaussianBlurImplementator
{
    public static class DllImporter
    {
        ...
        //public static extern int BlurOneAsm(byte[] source, byte[] destination, int width, int height, int offset, float[] kernel, float kernelSum, int kernelSize);

        [DllImport("D:\\GaussianBlurProject\\GaussianBlur\\x64\\Debug\\GaussianBlurAsm.dll")]
        public static extern int BlurOneAsm(byte[] source, byte[] destination, int width, int height, int offset);

        [DllImport("D:\\GaussianBlurProject\\GaussianBlur\\x64\\Debug\\GaussianBlurCpp.dll")]
        public static extern void BlurTarget(byte[] source, byte[] destination, int width, int height, int offset);
        ...
        public static extern void BlurOneAsm(byte[] source, byte[] destination, int width, int height, int offset);
